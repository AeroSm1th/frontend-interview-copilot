      ...
      <div className="space-y-6">
        ...
        {hasJdDraft ? (
          ...
          >
            ...
            <p className="mt-3 text-sm leading-7 text-zinc-600">{previewText}</p>
          </div>
        ) : (
        ...
        {isMatching || (!isMatching && !match && hasValidJdDraft && !errorMessage) || match ? (
          <div className="border-t border-zinc-100 pt-6">
            ...
            {!isMatching && !match && hasValidJdDraft && !errorMessage ? (
              <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 px-5 py-5">
                <div className="space-y-2">
                  <h2 className="text-base font-semibold text-zinc-900">还没有 JD 匹配结果</h2>
                  <p className="text-sm leading-7 text-zinc-600">
                    点击“匹配分析”后，这里会展示匹配度、命中技能、缺失技能、风险点和改进建议。
                  </p>
                </div>
              </div>
            ) : null}
